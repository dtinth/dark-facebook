...
function createMapping(yamls) {
  ...
  yamls.forEach(function(yaml) {
    ;(function recurse(components, prefix, parentSelector) {
      Object.keys(components).forEach(function(key) {
        var name = key.replace(/\(.+$/, '')
        var component = components[key]
        var name = prefix + name
        if (component.selector) {
          var selector = component.selector.replace(/&/g, parentSelector)
          ;(out[name] || (out[name] = [])).push(selector)
        }
        if (component.children) recurse(component.children, name + '--', selector)
      })
    })(yaml, '', '')
  })
  return out
